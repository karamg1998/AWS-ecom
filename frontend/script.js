window.addEventListener('DOMContentLoaded',async (e)=>{
       var page=1;
    try{
        await axios.get(`http://13.232.243.73:3000/getAllproducts/?page=${page}`)
       .then(products=>{
        ...
        console.log(err);
        document.body.innerHTML+=`<h2>Something went wrong while getting products on DOM</h2>`
    }

    try{
        await axios.get(`http://13.232.243.73:3000/getCartItems/?page=${page}`)
       .then(products=>{
        console.log(products.data);
        var p=products.data;
        for(var i=0;i<p.products.length;i++)
        ...
        console.log(err);
        document.body.innerHTML+=`<h2>Something went wrong while getting cart item on DOM</h2>`
    }
});
...
function newButton(data)
{
    ...
    for(var i=0;i<pagination.length;i++)
    {
        ...
        Page.addEventListener('click',(e)=>{
            e.preventDefault();
          axios.get(`http://13.232.243.73:3000/getAllproducts/?page=${e.target.innerText}`)
            .then(products=>{
                var p=products.data;
                ...
                console.log(err)
                console.log('error in main page pagination');
            });
        })
    }
};
...
function newCartButton(data)
{
    ...
    for(var i=0;i<cartPagination.length;i++)
    {
        ...
        CartPage.addEventListener('click',(e)=>{
            ...
            axios.get(`http://13.232.243.73:3000/getCartItems/?page=${e.target.innerText}`)
            .then(products=>{
                var p=products.data;
                ...
                console.log(err);
                console.log('error in cart pagination');
            });
        })
    }
};
...
function cartSideways()
{
    ...
    axios.get('http://13.232.243.73:3000/getCartItems/?page=1')
    .then(items=>{
...
async function showItems(data)
{
    ...
    for(var i=0;i<addToCart.length;i++)
    {
        addToCart[i].addEventListener('click',async (e)=>{
          ...
          try{
              var Atc=await axios.post('http://13.232.243.73:3000/addProductToCart',obj);
              popup(Atc.data.name);
          }
          catch(err){
            console.log(err);
...
async function remove_item(e)
{
    ...
    if(e.target.classList.contains('remove'))
    {
        ...
        try{
          ...
          let DelItems=await axios.delete(`http://13.232.243.73:3000/getCartItems/${id}`);
...
purchase.addEventListener('click',async (e)=>{
    ...
    try{
      await  axios.get('http://13.232.243.73:3000/getCartOrder')
        .then(items=>{
...
async function sendOrder(item)
{
    ...
    try{
     await   axios.post('http://13.232.243.73:3000/PostOrder',order)
        .then(res=>{
        console.log(res,'posted');
        });

    await axios.delete(`http://13.232.243.73:3000/getCartItems/${item.id}`)
    .then(res=>{
...
orders.addEventListener('click',async (e)=>{
    ...
    try{
    await axios.get(`http://13.232.243.73:3000/getOrders?page=1`)
    .then(orders=>{
      ...
      for(var i=0;i<orders.data.products.length;i++)
      {
        ...
        console.log(err);
        console.log('error in order pagination');
    }
       ...
       let oPage=document.querySelector('.order-pagination');
        ...
        for(var i=0;i<page.length;i++)
        {
            page[i].addEventListener('click',async (e)=>{
                ...
                o.innerHTML='';
                try{
                   await axios.get(`http://13.232.243.73:3000/getOrders?page=${e.target.innerText}`)
                .then(order=>{
                ...
                {
                    console.log(err);
                    console.log('error in loading orders using pagination');
                }
            })
        }
    }
